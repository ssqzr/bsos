...
function open_vm_tools::trait::post_install() {
    ...
    cmd::run_cmd_with_history --sudo -- echo "'vmhgfs-fuse /mnt/hgfs fuse defaults,allow_other 0 0'" ">>" "/etc/fstab" || return "${SHELL_FALSE}"
...
function open_vm_tools::trait::post_uninstall() {
    ...
    cmd::run_cmd_with_history --sudo -- sed -i '/vmhgfs-fuse/d' "/etc/fstab" || return "${SHELL_FALSE}"
    return "${SHELL_TRUE}"
}
