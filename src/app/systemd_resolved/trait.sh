...
function systemd_resolved::trait::post_install() {
    ...
    cmd::run_cmd_with_history --sensitive="${ROOT_PASSWORD}" -- printf "${ROOT_PASSWORD}" "|" su - root -c "\"ln -sf /run/systemd/resolve/stub-resolv.conf /etc/resolv.conf\"" || return "${SHELL_FALSE}"
    cmd::run_cmd_with_history --sensitive="${ROOT_PASSWORD}" -- printf "${ROOT_PASSWORD}" "|" su - root -c "\"systemctl restart systemd-resolved.service\"" || return "${SHELL_FALSE}"
    return "${SHELL_TRUE}"
}
